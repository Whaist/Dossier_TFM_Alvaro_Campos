...
%% SECUNDÁRIO! TESTES Transformar dados para colocar na rede neuronal (Exemplo com MÈDIA) 
j=1
T = [];
X = [];
for i = 1:length(VALC)
...
%% SECUNDÁRIO! TESTES Transformar dados para colocar na rede neuronal (Exemplo com numero de scatters)
j=1
T = [];
X = [];
for i = 1:length(VALC)
...
%% Criar primeira rede neuronal para classificar os três tipos de embarcações
...
%% Treinar a segunda rede neuronal para classificação de CARGO E TANKER vessels
% adicionar fator random aos dados
% É utilizado a mesma seed porque a correspondecia dos dados é relevante.
% É importante ser mensionado que como é feito a randomização dos dados 
% cada iteração produz resultados diferentes
